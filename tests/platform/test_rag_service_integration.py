...
def test_rag_retrieve_with_rerank(query):
    """Test retrieve() with reranking enabled."""
    response = retrieve(query=query, augmentation="expansion", rerank=True)
...
@pytest.mark.integration
@pytest.mark.rag
def test_rag_retrieve_without_rerank():
    """Test retrieve() with rerank disabled."""
    query = "What is the use of SD-WAN in LAT-Mesh?"
    response = retrieve(query=query, augmentation="hyde", rerank=False)
    assert isinstance(response, str)
    assert len(response.strip()) > 0
...
def test_rag_retrieve_empty_query():
    ...
    response = retrieve(query="", augmentation="expansion", rerank=True)
    assert isinstance(response, str)
...
@pytest.mark.integration
@pytest.mark.rag
def test_rag_retrieve_nonsense_query():
    """Test retrieve() with a nonsensical query."""
    query = "asdfghjklqwertyuiopzxcvbnm"
    response = retrieve(query=query, augmentation="hyde", rerank=True)
    assert isinstance(response, str)
...
@pytest.mark.integration
@pytest.mark.rag
def test_rag_retrieve_invalid_augmentation():
    """Test retrieve() with invalid augmentation fallback."""
    query = "What is microsegmentation?"
    response = retrieve(query=query, augmentation="unknown_type", rerank=True)
    assert isinstance(response, str)
    assert len(response.strip()) > 0
