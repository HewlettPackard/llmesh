...
def sample_doc_path():
    ...
    return "self_serve_platform/tests/unit/test_data/sample_doc_for_test.docx"
...
@patch('self_serve_platform.rag.data_extractors.markitdown_for_sections.FileCache')
@patch('self_serve_platform.rag.data_extractors.markitdown_for_sections.Logger.get_logger')
def test_parse_html_success(mock_get_logger, mock_file_cache, extractor, sample_html_path):  # pylint: disable=W0621
    ...
    with patch(
        'self_serve_platform.rag.data_extractors.markitdown_for_sections.MarkItDown'
    ) as mock_markitdown:
